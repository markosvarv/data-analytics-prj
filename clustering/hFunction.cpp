...
double hFunction::hValue (const vector<double>& p, int metric) const{
    if (p.size()!=v.size()) {
        cerr << "Error: different sizes of vectors in hFunction" << endl;
        //cout << p.size() << ' ' << this->v.size() << endl;
        return -1.0;
    }
...
void hFunction::add_value_to_cubeMap (int hvalue) {
    auto it = cube_map.find(hvalue);
    if (it == cube_map.end()) {
        cube_map[hvalue]=rand()%2;
    }
}

int hFunction::get_value_from_cubeMap (int hvalue) {
    return cube_map[hvalue];
}